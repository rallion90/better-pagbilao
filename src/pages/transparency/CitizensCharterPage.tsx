import { BookOpenCheck, ExternalLink, Landmark } from "lucide-react"
import { useLanguage } from "../../i18n/useLanguage"
import { useFormsAndDocuments } from "../../hooks/useFormsAndDocuments"
import { useSeo } from "../../hooks/useSeo"
import PageBreadcrumb from "../../components/common/PageBreadcrumb"

const PAGE_PATH = "/transparency/citizens-charter"

const CitizensCharterPage = () => {
    const { t } = useLanguage()
    const { data: forms, loading, error } = useFormsAndDocuments()
    const copy = t.transparencyPages.citizensCharter

    const charterDocs = forms?.transparency.filter((item) => item.title === "Citizen's Charter") ?? []

    const title = `${copy.heading} | Better Pagbilao Transparency`
    const description = "Pagbilao's Citizen's Charter — official service standards, transaction timelines, and frontline service guidance."
    ...
    useSeo({
        ...
        path: PAGE_PATH,
        jsonLd:
            forms && charterDocs.length > 0
                ? {
                      "@context": "https://schema.org",
                      "@type": "DigitalDocument",
                      name: "Citizen's Charter",
                      url: charterDocs[0].url,
                      about: {
                            ...
                            {charterDocs.length > 0 ? (
                                <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                                    {charterDocs.map((item) => (
                                        ...
                                        >
                                            <BookOpenCheck className="h-5 w-5 shrink-0 text-bayan-blue" />
...
export default CitizensCharterPage
